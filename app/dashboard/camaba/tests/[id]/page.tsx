...
export default function CamabaTestPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:4000/api/public/tests/${id}/submit`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          // Tidak pakai auth karena ini endpoint public
        },
      ...
      <div className="space-y-6">
        {test.questions.map((q: any, idx: number) => (
          <div key={q.id} className="p-4 border rounded-lg shadow-sm bg-white">
            ...
            {/* === JIKA ESSAY === */}
         {q.questionType?.toUpperCase() === "ESSAY" ? (
              <textarea className="w-full p-3 border rounded-lg bg-white" rows={4} placeholder="Tulis jawaban Anda di sini..." value={answers[q.id] || ""} onChange={(e) => handleSelect(q.id, e.target.value)} />
            ) : (
              /* === PILIHAN GANDA === */
