...
export default function TestResultPage() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:4000/api/test/${id}/result?attemptId=${attemptId}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((r) => r.json())
  ...
  return (
    <div className="max-w-2xl mx-auto p-8 text-black">
      <h1 className="text-3xl font-bold mb-4">
        Hasil Tes {test.type === "COLLEGE_READINESS" ? "College Readiness" : "Digital Literacy"}
      </h1>

      <div className="bg-white border rounded-xl p-6 shadow">
        <p><b>Skor Otomatis:</b> {attempt.autoScore}</p>
        <p><b>Final Skor:</b> {attempt.finalScore ?? "Menunggu penilaian"}</p>

        {attempt.passStatus && (
          <p className={`font-bold ${attempt.passStatus === "PASS" ? "text-green-600" : "text-red-600"}`}>
            {attempt.passStatus}
          </p>
        )}
      </div>
